...
class ProductionWorkerTester:
    ...
    async def test_rag_with_real_content(self):
        ...
        # Get user ID from the uploaded document
        user_id = "936551b6-b7a4-4d3d-9fe0-a491794fd66b"  # From previous tests
        ...
        for query in test_queries:
            ...
            try:
                ...
                print(f"     Response: {response.direct_answer[:100]}...")
                print(f"     Retrieved {len(chunks)} chunks")
                
                # Check if we got real content (not mock)
                has_real_content = any(
                    "deductible" in chunk.content.lower() or 
                    "coverage" in chunk.content.lower() or
                    "policy" in chunk.content.lower() or
                    "insurance" in chunk.content.lower()
                    for chunk in chunks
                )
                
                if has_real_content:
                    print(f"     ✅ Found real insurance content!")
                else:
                    print(f"     ⚠️  No real insurance content found")
        ...
        results_file = f"production_worker_test_results_{timestamp}.json"
