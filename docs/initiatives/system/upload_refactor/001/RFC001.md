...
This RFC builds upon **PRD001.md** to define the technical architecture for Insurance Navigator's insurance document ingestion pipeline refactor. The design implements the sequential processing pipeline (upload → parse → chunk → embed → finalize) with robust error handling, idempotent processing, and comprehensive observability.
