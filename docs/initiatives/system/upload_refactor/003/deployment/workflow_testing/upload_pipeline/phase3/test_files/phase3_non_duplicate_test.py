...
async def test_non_duplicate_pipeline():
    """Test with unique content to avoid duplicate detection"""
    
    # Database connection
    database_url = os.getenv('POOLER_URL', '${DATABASE_URL}/pdf", 1024, unique_hash, "files/test/unique_test.pdf", "uploaded", datetime.utcnow(), datetime.utcnow())
        ...
        if final_result:
            ...
            if pipeline_complete and all_stages_completed:
                print("\n✅ PHASE 3 NON-DUPLICATE TEST: SUCCESS!")
                print("   Complete pipeline processing verified!")
                return True
            else:
                print("\n❌ PHASE 3 NON-DUPLICATE TEST: FAILED!")
...
if __name__ == "__main__":
    asyncio.run(test_non_duplicate_pipeline())
