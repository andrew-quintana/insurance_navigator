...
export async function loadTestEnvironment(): Promise<EnvironmentConfig> {
  try {
    ...
    if (override) {
      ...
      // 3. Load base environment as fallback
      try {
        const baseEnv = await Deno.readTextFile('.env');
        applyEnvFile(baseEnv);
      } catch (error) {
        console.log('No base .env file found, using only test environment');
      }
    }

    // 4. Validate and return environment config
    ...
    storage: {
      from: () => ({
        ...
        remove: () => Promise.resolve({
          ...
          error: mockData.removeError || null
        })
      })
    },
    from: (table: string) => ({
