...
-- Add scheduled job to run job-processor every minute
select cron.schedule(
  ...
  select
    net.http_post(
      url:= (select value from secrets.decrypted_secrets where name = 'APP_PROJECT_URL') || '/functions/v1/job-processor',
      headers:=jsonb_build_object(
        'Content-type', 'application/json',
        'Authorization', 'Bearer ' || (select value from secrets.decrypted_secrets where name = 'APP_SERVICE_ROLE_KEY')
      ),
