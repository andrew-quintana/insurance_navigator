...
async function handleWebhook(req, supabase) {
  ...
  const isLlamaParseWebhook = webhookData.txt || webhookData.md || webhookData.json || !webhookData.source && webhookData.txt !== undefined;
  ...
  if (webhookData.documentId) {
    const { data, error } = await supabase.from('documents').select('*').eq('id', webhookData.documentId).single();
    if (error) {
    ...
    const { data, error } = await supabase.from('documents').select('*').eq('llama_parse_job_id', webhookData.jobId).single();
    if (error) {
      console.error('Document lookup by job ID error:', error);
      return new Response(JSON.stringify({
        error: 'Document not found by job ID'
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    return new Response(JSON.stringify({
      error: 'Document ID or Job ID required'
    }), {
      status: 400,
      headers: {
        ...corsHeaders,
...
async function handleLlamaParseWebhook(req, supabase, webhookData) {
  ...
  try {
    ...
    const { data: recentDocuments, error: searchError } = await supabase.from('documents').select('*').is('llama_parse_job_id', null).eq('status', 'processing').order('created_at', {
      ascending: false
    }).limit(10);
    if (searchError || !recentDocuments || recentDocuments.length === 0) {
      ...
      const { data: fallbackDocs } = await supabase.from('documents').select('*').eq('status', 'processing').order('created_at', {
        ascending: false
      }).limit(1);
    ...
    if (!document) {
      return new Response(JSON.stringify({
        error: 'No document found for processing'
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    const updateData = {
      ...
      llama_parse_job_id: `llamaparse_${Date.now()}`,
    ...
    // Update document
    const { error: updateError } = await supabase.from('documents').update(updateData).eq('id', document.id);
    if (updateError) {
...
async function handleStorageWebhook(supabase, document, webhookData) {
  const updateData = {
    updated_at: new Date().toISOString()
  };
...
async function handleEmbeddingWebhook(supabase, document, webhookData) {
  const updateData = {
...
async function handleInternalWebhook(supabase, document, webhookData) {
...
// Helper functions
async function triggerChunking(supabase, documentId, parseResult) {
...
async function triggerLlamaParseJob(supabase, documentId) {
...
function needsLlamaParse(contentType) {
  return contentType === 'application/pdf' || contentType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
}
async function checkFeatureFlag(supabase, flagName, userId) {
...
async function sendProgressUpdate(supabase, userId, update) {
  try {
    await supabase.from('realtime_progress_updates').insert({
      user_id: userId,
