...
class CloudIntegrationTester:
    ...
    async def _test_api_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(f"{self.config['api_url']}/api/upload-pipeline/upload", timeout=30) as response:
    ...
    async def _test_document_upload_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(f"{self.config['api_url']}/api/upload-pipeline/upload", data=data, timeout=60) as response:
                upload_result = {
                    'status_code': response.status,
