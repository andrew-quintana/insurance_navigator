...
def quick_health_check():
    ...
    try:
        response = requests.get('https://insurance-navigator-api.onrender.com/health', timeout=5)
...
def quick_upload_test(token):
    ...
    try:
        response = requests.post(
            'https://insurance-navigator-api.onrender.com/upload-regulatory-document',
            headers=headers,
            files=files,
            data=data,
            timeout=30
