...
async def test_terminal_stage_finalization(file_path):
    """Test pipeline processing with terminal stage finalization."""
    print(f"\n🚀 Testing Terminal Stage Finalization: {os.path.basename(file_path)}")
    ...
    try:
        ...
        document_id, job_id = await create_test_document(pool, file_path, TEST_USER_ID)
        ...
        config = WorkerConfig(
            ...
            terminal_stage="embedded"  # Explicit terminal stage
        )
        
        # Create worker
        print(f"🔄 Starting pipeline processing (terminal stage: {config.terminal_stage})...")
        ...
        # Process jobs until terminal stage or max iterations
        max_iterations = 15
        iteration = 0
        ...
        # Determine success
        success = (job_status and 
                  job_status['stage'] == config.terminal_stage and 
                  job_status['state'] == 'done' and
                  doc_status and 
                  doc_status['processing_status'] == 'completed')
        
        if success:
            print(f"\n✅ SUCCESS: Terminal stage finalization achieved!")
            print(f"   Document processed to '{config.terminal_stage}' stage with state 'done'")
            print(f"   Document status is 'completed'")
            print(f"   Generated {chunk_count} chunks")
            return True
        else:
            print(f"\n⚠️  FAILURE: Terminal stage finalization not achieved")
            print(f"   Expected: stage='{config.terminal_stage}', state='done'")
            print(f"   Actual: stage='{job_status['stage'] if job_status else 'UNKNOWN'}', state='{job_status['state'] if job_status else 'UNKNOWN'}'")
            return False
...
async def verify_no_chunks_buffered_references():
    """Verify no chunks_buffered references remain in active code."""
    print("\n🔍 Verifying no 'chunks_buffered' references remain...")
    
    # Check main worker files
    worker_files = [
    ...
    if not chunks_buffered_found:
        print("   ✅ SUCCESS: No 'chunks_buffered' references found in worker code")
        return True
    else:
        print("   ❌ FAILURE: 'chunks_buffered' references still exist")
        return False

async def main():
    """Run closeout validation tests."""
    print("🏁 Starting Closeout Validation Tests")
    print("="*60)
    
    # Verify stage name cleanup
    stage_cleanup_success = await verify_no_chunks_buffered_references()
    
    # Test terminal stage finalization with both files
    all_tests_passed = True
    for test_file in TEST_FILES:
        if os.path.exists(os.path.join(project_root, test_file)):
            file_success = await test_terminal_stage_finalization(test_file)
            all_tests_passed = all_tests_passed and file_success
        else:
    ...
    # Final results
    print("\n" + "="*60)
    print("🏁 CLOSEOUT VALIDATION RESULTS")
    print("="*60)
    
    if stage_cleanup_success:
        print("✅ Stage rename: 'chunks_buffered' → 'chunked' completed")
    else:
        print("❌ Stage rename: 'chunks_buffered' references still exist")
    
    if all_tests_passed:
        print("✅ Terminal stage finalization: All tests passed")
        print("✅ State transitions: All jobs reached 'done' state")
        print("✅ Document processing: All documents marked 'completed'")
        print("\n🎉 ALL CLOSEOUT REQUIREMENTS MET!")
        return True
    else:
        print("❌ Terminal stage finalization: Some tests failed")
        print("\n⚠️  CLOSEOUT REQUIREMENTS NOT FULLY MET")
        return False
