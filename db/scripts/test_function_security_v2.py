#!/usr/bin/env python3
"""
Function Security Testing Script (Version 2)

Updated version that properly handles authentication context and tests
the improved security functions.

Usage:
    python test_function_security_v2.py [--database-url DATABASE_URL]
"""
...
class FunctionSecurityTester:
    ...
    async def setup_test_data(self):
        ...
        try:
            ...
            await self.connection.execute("SELECT public.clear_user_context()")
    ...
    async def test_function_security_definer_usage(self) -> List[SecurityTestResult]:
        ...
        try:
            ...
            functions = await self.connection.fetch("""
                ...
                AND proname IN ('is_admin', 'get_current_user_id', 'has_role', 'can_access_policy', 'set_current_user_context', 'log_policy_access', 'clear_user_context')
            """)
    ...
    async def test_admin_function_logic(self) -> List[SecurityTestResult]:
        ...
        try:
            # Test 1: Clear context and check that is_admin returns false
            await self.connection.execute("SELECT public.clear_user_context()")
    ...
    async def test_user_id_function_security(self) -> List[SecurityTestResult]:
        ...
        try:
            # Test 1: Clear context and check that function returns null
            await self.connection.execute("SELECT public.clear_user_context()")
            ...
            results.append(SecurityTestResult(
                ...
                security_level="HIGH",
                vulnerability_type="authentication_bypass" if result is not None else None,
    ...
    async def test_function_input_validation(self) -> List[SecurityTestResult]:
        ...
        try:
            ...
            # Test has_role with null input (should return false)
            ...
            # Test has_role with empty string (should return false)
            ...
            # Test has_role with no user context (should return false)
            result = await self.connection.fetchval("SELECT public.has_role('admin')")
            
            results.append(SecurityTestResult(
                test_name="has_role() returns false without user context",
                passed=result is False,
                security_level="HIGH",
                error_message=None if result is False else f"Function should return false without user context: {result}"
            ))
    ...
    async def test_context_isolation(self) -> List[SecurityTestResult]:
        """Test that user context is properly isolated between sessions."""
        results = []
        
        try:
            # Test 1: Set user context and verify it's set
            await self.connection.execute(
                "SELECT public.set_current_user_context($1)", 
                self.test_user_id
            )
            ...
            results.append(SecurityTestResult(
                test_name="User context can be set correctly",
                passed=str(result) == self.test_user_id,
                security_level="HIGH",
                error_message=None if str(result) == self.test_user_id else f"Context not set correctly: {result}"
            ))
            
            # Test 2: Clear context and verify it's cleared
            await self.connection.execute("SELECT public.clear_user_context()")
            result = await self.connection.fetchval("SELECT public.get_current_user_id()")
            
            results.append(SecurityTestResult(
                test_name="User context can be cleared correctly",
                passed=result is None,
                security_level="HIGH",
                error_message=None if result is None else f"Context not cleared: {result}"
            ))
            
            # Test 3: Verify admin status changes with context
            await self.connection.execute("SELECT public.clear_user_context()")
            result = await self.connection.fetchval("SELECT public.is_admin()")
            
            results.append(SecurityTestResult(
                test_name="Admin status reflects cleared context",
                passed=result is False,
                security_level="CRITICAL",
                error_message=None if result is False else f"Admin status should be false with no context: {result}"
            ))
            
        except Exception as e:
            results.append(SecurityTestResult(
                test_name="Context isolation test",
                passed=False,
                security_level="HIGH",
                vulnerability_type="unknown",
                error_message=f"Error testing context isolation: {e}"
            ))
    ...
    async def run_all_security_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_results.extend(await self.test_context_isolation())
...
async def main():
    ...
    try:
        ...
        report_file = f'function_security_report_v2_{timestamp}.json'
