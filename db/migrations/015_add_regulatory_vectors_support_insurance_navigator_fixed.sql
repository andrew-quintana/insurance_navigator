-- =============================================================================
-- Migration: Add Regulatory Document Vector Support (Final - insurance_navigator)
-- Version: 015 Insurance Navigator Fixed
-- Description: Enhance user_document_vectors table to support regulatory documents
-- =============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'user_document_vectors_regulatory_document_id_fkey'
    ) THEN
        ALTER TABLE user_document_vectors 
        ADD CONSTRAINT user_document_vectors_regulatory_document_id_fkey 
...
-- Update existing records based on their data (using document_id instead of document_record_id)
UPDATE user_document_vectors 
SET document_source_type = CASE 
    WHEN document_id IS NOT NULL THEN 'user_document'
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'user_document_vectors_document_source_type_check'
    ) THEN
        ALTER TABLE user_document_vectors 
        ADD CONSTRAINT user_document_vectors_document_source_type_check 
        CHECK (document_source_type IN ('user_document', 'regulatory_document'));
    END IF;
END $$;

-- Rename chunk_embedding to content_embedding for consistency (if needed)
-- Note: This keeps the existing chunk_embedding column for compatibility
-- The processor can use content_embedding field name but store in chunk_embedding column

-- Create indexes for regulatory document vectors
CREATE INDEX IF NOT EXISTS idx_user_document_vectors_regulatory_doc 
ON user_document_vectors(regulatory_document_id) 
WHERE regulatory_document_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_user_document_vectors_source_type 
ON user_document_vectors(document_source_type);

-- Composite index for efficient regulatory vector queries
CREATE INDEX IF NOT EXISTS idx_user_document_vectors_regulatory_search 
ON user_document_vectors(regulatory_document_id, document_source_type, is_active) 
