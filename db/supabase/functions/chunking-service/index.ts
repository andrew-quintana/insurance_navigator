...
serve(async (req) => {
  ...
  try {
    console.log('🚀 Starting chunking service...')
    ...
    const { documentId, text }: ChunkRequest = JSON.parse(requestText)
    console.log('📄 Parsed request:', { documentId, hasText: !!text, textLength: text?.length })
