...
interface RealtimePayload {
  type: 'progress_update' | 'status_change' | 'error' | 'complete';
  documentId: string;
  progress: number;
  status: string;
  timestamp: string;
...
async function handleProgressUpdate(req: Request, supabase: any) {
  const progressData: ProgressUpdate = await req.json()
  ...
  if (progressData.metadata) {
    updateData.metadata = { ...document.metadata, ...progressData.metadata }
  }
...
async function handleProgressQuery(req: Request, supabase: any) {
  ...
  let query = supabase
    .from('documents')
    .select('id, status, progress_percentage, processed_chunks, total_chunks, failed_chunks, error_message, original_filename, file_size, created_at, updated_at')
    .eq('id', documentId)
  ...
  const isStuck = document.status === 'processing' && 
    new Date().getTime() - new Date(document.updated_at).getTime() > 1800000 // 30 minutes
...
async function handleProgressReset(req: Request, supabase: any) {
  const url = new URL(req.url)
  const documentId = url.searchParams.get('documentId')
...
function determineUpdateType(progressData: ProgressUpdate, document: any): RealtimePayload['type'] {
...
async function sendRealtimeUpdate(supabase: any, userId: string, payload: RealtimePayload) {
...
function calculateETA(document: any): number | null {
